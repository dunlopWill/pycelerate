...
No `f"={a}-{b}"` string assembly, no tracking which cell a value went to, and no
guessing about parentheses.

## Why

Hand-assembled formula strings get precedence wrong silently. The obvious version of
"just concatenate the coordinates" turns `(revenue - cost) / revenue` into
`=B3-B4/B3` — a wrong number, not an error. pycelerate builds a small expression
tree and renders it with Excel's precedence rules, so the parentheses are right and
there are no redundant ones.
...
as dict keys and in sets. Reflected operators work too, so `1000 - cost` is fine.
...
**Functions added in Excel 2010 or later are prefixed automatically.** The file
format stores these as `_xlfn.XLOOKUP`, `_xlfn.TEXTJOIN`, `_xlfn._xlws.FILTER` and
so on; written bare they show up as `#NAME?` when the workbook is opened. You write
the plain name and get the right thing:
